...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error ?? 'Account initialization failed.');
    } finally {
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#050505] p-6">
      <div className="w-full max-w-[500px]">
        <div className="flex justify-between items-center mb-8">
          <Link
            href="/login"
            className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-zinc-500 hover:text-white group"
          >
            ...
            {error && (
              <div className="p-4 rounded-xl bg-red-500/5 border border-red-500/20 text-center space-y-3">
                ...
                {error.toLowerCase().includes('registered') && (
                  <Link
                    href="/login"
                    className="flex items-center justify-center gap-2 text-white bg-red-500/20 py-2 rounded-lg text-[10px] font-bold uppercase hover:bg-red-500/30 transition-all"
                  >
