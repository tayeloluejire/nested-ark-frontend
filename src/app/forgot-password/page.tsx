...
export default function ForgotPasswordPage() {
  ...
  const handleRecovery = async (e: React.FormEvent) => {
    ...
    try {
      // Adjusted to /auth/forgot-password to match standard backend nested routing
      await api.post('/auth/forgot-password', { email });
      setSubmitted(true);
    } catch (err: any) {
      console.error('Recovery Protocol Error:', err);
      // Fallback message if backend doesn't provide one
      const message = err.response?.data?.message || 'Protocol failed. Verify email and retry.';
      setError(message);
    } finally {
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
          {!submitted ? (
            ...
            <div className="text-center py-4 space-y-4 animate-in fade-in zoom-in duration-300">
              ...
              <p className="text-[10px] text-teal-500 uppercase tracking-widest font-bold">Transmission Complete. Check Inbox/Spam.</p>
            </div>
          )}
        </div>
