...
// ── Tri-Layer Verification Status Badge ──────────────────────
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-3xl p-8 space-y-6">
        ...
        <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 space-y-2">
          ...
          <p className="text-[10px] text-zinc-600">Upload to Cloudinary, S3, or any public URL. AI validator will analyse the file automatically.</p>
        </div>
        <div className="space-y-3">
          <input
            type="url"
            placeholder="https://your-storage.com/evidence-photo.jpg"
            value={evidenceUrl}
        ...
        <div className="flex gap-3">
          <button onClick={submitEvidence} disabled={submitting || !evidenceUrl} className="flex-1 py-3 bg-white text-black font-bold uppercase text-xs tracking-widest rounded-xl hover:bg-teal-500 transition-all disabled:opacity-40 flex items-center justify-center gap-2">
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-3xl p-8 space-y-6">
        ...
        <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/20">
          ...
          <p className="text-[10px] text-zinc-500 mt-1">Review the evidence, visit the site if required, then approve or reject. This decision is logged immutably.</p>
        </div>
        <textarea rows={3} value={notes} onChange={e => setNotes(e.target.value)} placeholder="Audit notes, observations, inspection summary..." className="w-full bg-black border border-zinc-800 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-teal-500 transition-colors resize-none" />
        <div className="grid grid-cols-2 gap-3">
          <button onClick={() => submitAudit(true)} disabled={submitting} className="py-3 bg-teal-500 text-black font-bold uppercase text-xs tracking-widest rounded-xl hover:bg-teal-400 transition-all disabled:opacity-40 flex items-center justify-center gap-2">
            {submitting ? <Loader2 className="animate-spin" size={14} /> : <><CheckCircle2 size={12} /> Approve</>}
          </button>
          <button onClick={() => submitAudit(false)} disabled={submitting} className="py-3 bg-red-500/20 text-red-400 border border-red-500/30 font-bold uppercase text-xs tracking-widest rounded-xl hover:bg-red-500/30 transition-all disabled:opacity-40 flex items-center justify-center gap-2">
...
export default function MilestonesPage() {
  ...
  const handleRelease = async (m: any) => {
    ...
    const skipDrone = !allVerified && m.drone_status !== 'VERIFIED';
    ...
    if (!allVerified) {
      ...
      const bypass = confirm(
        `⚠ WARNING: Not all verification layers are complete.\nMissing: ${missing}\n\nProceed anyway? (Drone layer will be marked as skipped)`
      );
      if (!bypass) return;
    } else {
    ...
    try {
      ...
      if (gateInfo) {
        alert(`🔒 BLOCKED: ${err.response.data.error}\nBlocked by: ${gateInfo.blocked_by?.join(', ')}`);
      } else {
    ...
    <div className="min-h-screen bg-[#050505] text-white">
      {evidenceModal && (
        <EvidenceModal
          ...
          onSuccess={() => { setEvidenceModal(null); mutate(); }}
        />
      )}
      {auditModal && (
        <AuditModal
          milestone={auditModal}
          onClose={() => setAuditModal(null)}
          onSuccess={() => { setAuditModal(null); mutate(); }}
        />
      )}
                ...
                <div className="flex flex-col md:flex-row justify-between gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 flex-wrap">
                        <span className={`text-[9px] px-2 py-0.5 rounded border uppercase font-bold ${
                          ...
                          allVerified ? 'border-teal-500/30 text-teal-400' :
                        ...
                        {allVerified && !isPaid && (
                          <span className="text-[9px] text-teal-500 font-bold uppercase">🔓 Release Ready</span>
                        )}
                      </div>

                      {/* Tri-layer verification bars */}
                      ...
                      {/* Evidence hash if validated */}
                      {m.ai_hash && (
                        <p className="text-[8px] text-zinc-700 font-mono mt-2 truncate" title={m.ai_hash}>
                          AI Hash: {m.ai_hash.slice(0, 24)}…
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col justify-between items-end gap-4 min-w-[160px]">
                    ...
                    <div className="flex flex-col gap-2 w-full">
                      ...
                      {canRelease && (
                        <button onClick={() => handleRelease(m)} disabled={releasing === m.id} className="flex items-center justify-center gap-2 px-4 py-2 bg-white text-black font-black rounded-xl text-[10px] uppercase tracking-tighter hover:bg-teal-500 transition-all disabled:opacity-50">
                          {releasing === m.id ? <Activity className="animate-spin" size={12} /> : <><ArrowUpRight size={12} /> Release</>}
                        </button>
                      )}
                      {isPaid && (
                        <span className="text-teal-500 flex items-center justify-end gap-1 text-[10px] font-black uppercase tracking-widest">
                          <CheckCircle2 size={12} /> Dispatched
                        </span>
                      )}
                      {!isPaid && !canRelease && !canSubmitEvidence && !canAudit && (
                        <span className="text-zinc-600 flex items-center justify-end gap-1 text-[10px] font-bold uppercase tracking-widest italic">
