...
import { Activity, Briefcase, TrendingUp, CheckCircle2, Clock, DollarSign, Database, ShieldCheck, Loader2 } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(user.role === 'GOVERNMENT' || user.role === 'ADMIN') && (
            <button onClick={() => router.push('/projects')} className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/30 hover:border-teal-500/50 transition-all text-left">
              ...
              {projects.length > 0 && <p className="text-teal-500 text-xs mt-2 font-bold">{projects.length} project{projects.length !== 1 ? 's' : ''} active</p>}
            </button>
          )}
          {user.role === 'INVESTOR' && (
            <button onClick={() => router.push('/investments')} className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/30 hover:border-teal-500/50 transition-all text-left">
              ...
              {projects.length > 0 && <p className="text-emerald-500 text-xs mt-2 font-bold">{projects.length} node{projects.length !== 1 ? 's' : ''} available</p>}
            </button>
          )}
          {user.role === 'CONTRACTOR' && (
            <button onClick={() => router.push('/projects')} className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/30 hover:border-teal-500/50 transition-all text-left">
              ...
              {projects.length > 0 && <p className="text-amber-500 text-xs mt-2 font-bold">{projects.length} project{projects.length !== 1 ? 's' : ''} open</p>}
            </button>
          )}
          <button onClick={() => router.push('/milestones')} className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/30 hover:border-teal-500/50 transition-all text-left">
            ...
            {milestones.length > 0 && <p className="text-teal-500 text-xs mt-2 font-bold">{milestones.length} milestone{milestones.length !== 1 ? 's' : ''} tracked</p>}
          </button>
          ...
          {user.role === 'CONTRACTOR' && (
            <button onClick={() => router.push('/contractor-profile')} className="p-6 rounded-2xl border border-zinc-800 bg-zinc-900/30 hover:border-amber-500/50 transition-all text-left">
              <ShieldCheck className="text-amber-500 mb-4" size={24} />
              <h3 className="font-bold uppercase tracking-tight mb-1">My Profile</h3>
              <p className="text-zinc-500 text-xs">Create or update your contractor profile to unlock bidding.</p>
            </button>
          )}
        </div>
      </main>
